...
export function tripleNumbers(numbers: number[]): number[] {
    ...
    return newArr.map((num: number): number => num * 3);
}
...
export function stringsToIntegers(numbers: string[]): number[] {
    const newArr = [...numbers];
    return newArr.map((str: string): number =>
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    return messages
        .filter((str: string): boolean => str.charAt(str.length - 1) !== "?")
...
export function countShortWords(words: string[]): number {
    const count = words.filter((str: string): boolean => str.length < 4);
    return count.length;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const firstNegativeIndex = newArr.findIndex(
    ...
    const sum = newArr.slice(0, firstNegativeIndex).reduce((a, b) => a + b, 0);
